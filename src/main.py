#!/usr/bin/python3
"""
Many-to-many (i.e. sequence to sequence) time series modeling.
...
For many-to-many research and analysis.
...
if cfg.DATA >= 0:
    ...
    # must do these before reshape because need to know pre-reshape number to calculate ratio of data increase
    # only used in one other place, if used agin somewhere else, check this logic
    train_timesteps = train_X.shape[0]
    valid_timesteps = valid_X.shape[0]

    if cfg.SLIDE:
        if verbose: print("  shaping train... ", end="")
        train_X, train_Y, randnums_train = fn.shape_slide_3D_data(train_X, train_Y, cfg.SEQLENGTH, cfg.SEQLENGTH//cfg.RMAX, cfg.STEP, cfg.SEC2SEQ)
        if verbose: print("shaping valid... ", end="")
        valid_X, valid_Y, randnums_valid = fn.shape_slide_3D_data(valid_X, valid_Y, cfg.SEQLENGTH, cfg.SEQLENGTH//cfg.RMAX, cfg.STEP, cfg.SEC2SEQ)
        if verbose: print("shaping test... ", end="")
        test_X, test_Y, randnums_test = fn.shape_slide_3D_data(test_X,  test_Y, seqlen=cfg.SEQLENGTH, slide=1, seq2seq=cfg.SEC2SEQ)
    ...
    train_X, train_Y, valid_X, valid_Y, test_X, test_Y = \
        fn.generate_3D_data(num_classes = cfg.OUTPUT, sequence_lenghth = cfg.SEQLENGTH, verbose=verbose)
...
if verbose:
    ...
    if cfg.SEC2SEQ:
        ...
        print()
        print("sequences with transitions:")
        print("  train:", fn.transitions(train_Y))
        print("  valid:", fn.transitions(valid_Y))
        print("  test: ", fn.transitions(test_Y))
    else:
        print("  test_Y", test_Y.shape)
    print()
    print("Sequence length:", cfg.SEQLENGTH)
...
# must do this after reshape because train instances will be different (same as above comment)
if cfg.WEIGHTED:
    if verbose:
        print("Sample weights calculated.")
        print()
    sample_weights = fn.calculate_weights(train_Y)
else:
    if verbose:
        print("Sample weights NOT calculated ")
        print()
    sample_weights = None
...
model.compile(optimizer='adam',
              ...
              metrics=['accuracy'],
              run_eagerly=cfg.EAGERLY,
              sample_weight_mode="temporal" if cfg.WEIGHTED else None)
...
if cfg.PREDICT:
    ...
    ###############################
    if verbose: print("Post-processing predictions...", end="")
    ...
    if verbose:
        ...
        if cfg.SEC2SEQ:
            ...
            fn.print_stats(cfg.ACTUALPREDICTEDS, cfg.OUTPUT, save_cm="test")
        else:
            print(os.path.basename(cfg.ACTUALPREDICTEDV))
            fn.print_stats(cfg.ACTUALPREDICTEDV, cfg.OUTPUT, save_cm="test")
